...
現在の KAGI は `partially self-hosted` です。canonical selfhost frontend path は compile-once front half、typed bundle、KIR path、golden corpus まで進んでいますが、まだ `fully self-hosted` ではありません。

現時点で `fully self-hosted` と呼べない主な理由:

- canonical `compile_source_v1(...)` はまだ Python の bundle decoder を使う
  - `src/kagi/compile_result.py`
  - `src/kagi/selfhost_bundle.py`
- canonical selfhost frontend path はまだ Python の KIR executor を使う
  - `src/kagi/selfhost_runtime.py`
  - `src/kagi/kir_runtime.py`
- canonical compile path はまだ Python の string helper builtin 群に依存する
  - `src/kagi/subset_builtins.py`
  - 具体例: `trim`, `starts_with`, `extract_quoted`, `line_count`, `line_at`, `before_substring`, `after_substring`, `is_identifier`, `concat`, `eq`
...
この repo では、次の条件を満たした時点で `fully self-hosted` と呼びます。

- canonical selfhost compile/run path が Python bundle decoder を主経路で使わない
- canonical selfhost compile/run path が Python KIR executor を主経路で使わない
- canonical selfhost compile/run path が Python string helper builtin 群を主経路で使わない
