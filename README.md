...
現時点では parser/check/lower の一部を self-hosted 側へ寄せ始めており、単純な `print "..."` 1 行ケース、`let x = "..."` / `print x` の 2 行ケース、`fn name(x) { print concat(x, "...") }` + `call name("...")` の固定形は `examples/selfhost_frontend.ks` 自身が `ok` / AST / artifact を返します。より広い構文はまだ Python seed builtin に fallback します。
