...
いまは `print "..."` 文を受け付ける極小言語を対象にしています。複数行の `print` 文、`print concat("a","b")` のような最小式、`let x = ...` と `print x`、さらに `eq(...)` と `if(cond, then, else)`、`if cond { ... } else { ... }`、`fn name() { ... }` と `call name()` を処理できます。
