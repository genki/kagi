...
launcher/runtime 間の portable ABI 名は
...
native runtime の entry 契約は `argv[1]` ベースです。`KAGI_ENTRY_TARGET` は launcher からも export されますが、現在の native runtime bridge が実際に dispatch に使うのは `argv[1]` で、env 側は compatibility/debugging 用の境界として残しています。
