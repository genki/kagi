...
`compile_source_v1(...)` が、`parse -> check -> lower -> compile` を 1 つの typed result にまとめます。現時点の selfhost 主経路では frontend 実行は `parse` と `compile` に絞り、`check/lower` 相当は typed contract 側で導出しています。
...
`selfhost-run` は compile の返す artifact をそのまま実行します。非 JSON の主経路は `pipeline -> artifact -> stdout` です。
